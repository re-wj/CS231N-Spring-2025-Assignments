...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        pred_conditioned = self.forward(x, time, model_kwargs)
        model_kwargs["text_emb"] = torch.zeros_like(model_kwargs["text_emb"])
        pred_unconditioned = self.forward(x, time, model_kwargs)
        x = (cfg_scale+1) * pred_conditioned - cfg_scale * pred_unconditioned
