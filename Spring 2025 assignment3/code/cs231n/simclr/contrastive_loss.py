...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.sum(z_i * z_j, dim=-1) / (torch.linalg.norm(z_i, dim=-1) * torch.linalg.norm(z_j, dim=-1))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / (torch.sum(torch.exp(sim(z_k, out) / tau)) - np.exp(1 / tau)))
        total_loss += -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / (torch.sum(torch.exp(sim(z_k_N, out) / tau)) - np.exp(1 / tau)))
...
def compute_sim_matrix(out):
    ...
    sim_matrix = torch.matmul(out, out.transpose(-1, -2)) / torch.linalg.norm(out, dim=-1).unsqueeze(0) / torch.linalg.norm(out, dim=-1).unsqueeze(1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pair = torch.cat((sim_positive_pairs(out_left, out_right), sim_positive_pairs(out_right, out_left)), dim=0)
