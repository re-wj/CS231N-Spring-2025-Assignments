...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        x_start = (x_t - torch.sqrt(1 - alpha_cumprod) * noise) / torch.sqrt(alpha_cumprod)
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        alpha_cumprod = extract(self.alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - torch.sqrt(alpha_cumprod) * x_start) / torch.sqrt(1 - alpha_cumprod)
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #q(x_{t-1} | x_t, x_0) = \mathcal{N}(x_{t-1}; mu_t_tilde, beta_t_tilde I)
        posterior_mean, posterior_std = self.q_posterior(x_start, x_t, t)

        #p_theta(x_{t-1} | x_t) = \mathcal{N}(x_{t-1}, mu_theta, Sigma_theta)
    ...
    def q_sample(self, x_start, t, noise):
        ...
        #q(x_t | x_0) = \mathcal{N}(x_t; \sqrt{alpha_bar}x_0, \sqrt{1-alpha_bar}I)
        #x_t ~ \mathcal{N}(\sqrt{alpha_bar}x_0, \sqrt{1-alpha_bar}I)
        #x_t = \sqrt{alpha_bar}x_0 + \sqrt{1-alpha_bar} eps
        alpha_cumprod = extract(self.alphas_cumprod, t, x_start.shape)
        x_t = torch.sqrt(alpha_cumprod) * x_start + torch.sqrt(1-alpha_cumprod) * noise
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        #L = \mathbb{E}_{x_0, eps}[w_t * ||eps - eps_theta||^2]
