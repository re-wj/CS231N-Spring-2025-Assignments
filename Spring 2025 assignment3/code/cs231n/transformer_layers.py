...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe = torch.exp(-torch.arange(0, embed_dim, 2) / embed_dim * math.log(10000))
        pe = torch.matmul(torch.arange(max_len, dtype=pe.dtype).unsqueeze(dim=1), pe.unsqueeze(dim=0))
        pe = torch.stack((torch.sin(pe), torch.cos(pe)), dim=-1).reshape(1, max_len, embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        #(q_1, q_2, ..., q_h) = X(Q_1, Q_2, ..., Q_h)
        query = self.query(query)
        query = query.view(N, S, self.n_head, self.head_dim).transpose(1, 2)    #q: (N, H, S, E/H)
        #(k_1, k_2, ..., k_h) = X(K_1, K_2, ..., K_h)
        key = self.key(key)
        key = key.view(N, T, self.n_head, self.head_dim).transpose(1, 2)        #k: (N, H, T, E/H)
        #(v_1, v_2, ..., v_h) = X(V_1, V_2, ..., V_h)
        value = self.value(value)
        value = value.view(N, T, self.n_head, self.head_dim).transpose(1, 2)    #v: (N, H, T, E/H)

        #s = qk^T / \sqrt(E/H)
        attn_scores = torch.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.head_dim) #s: (N, H, S, T)
        #masked s: -inf
