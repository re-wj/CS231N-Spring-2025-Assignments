...
def get_similarity_no_loop(text_features, image_features):
    ...
    similarity = torch.matmul(text_features, image_features.transpose(-1, -2)) / torch.linalg.norm(text_features, dim=-1).unsqueeze(1) / torch.linalg.norm(image_features, dim=-1).unsqueeze(0)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    pred_classes_indices = torch.argmax(similarities, dim=0).tolist()
    pred_classes = [class_texts[i] for i in pred_classes_indices]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        images = torch.cat(images, dim=0).to(device)
        self.image_features = clip_model.encode_image(images)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        query = self.model.encode_text(query)
        similarities = get_similarity_no_loop(query, self.image_features).squeeze(0)
        _, top_indices = torch.topk(similarities, k=k)
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim: int = 384, lr: float = 0.001, weight_decay: float = 0):
        """
        ...
        self.model = nn.Sequential(
            nn.Linear(inp_dim, inp_dim // 4, device=device),
            nn.ReLU(),
            nn.Linear(inp_dim // 4, num_classes, device=device)
        )
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        batch_size = max(32, num_data // num_iters)
