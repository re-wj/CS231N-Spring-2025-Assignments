...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        embeds = word_embedding_forward(captions_in, W_embed)
        
        if self.cell_type == 'rnn':
            h = rnn_forward(embeds, h0, Wx, Wh, b)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(1, max_length):
            if end:
                captions[:, i] = self._null * torch.ones((N, 1), dtype=torch.long)
                continue
            embed = word_embedding_forward(token, W_embed)
            if self.cell_type == 'rnn':
                hidden_state = rnn_step_forward(embed, hidden_state, Wx, Wh, b)
            scores = temporal_affine_forward(hidden_state.unsqueeze(dim=1), W_vocab, b_vocab).squeeze(dim=1)
