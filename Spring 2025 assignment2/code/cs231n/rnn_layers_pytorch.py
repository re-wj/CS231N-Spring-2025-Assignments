...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = torch.nn.functional.tanh(x @ Wx + prev_h @ Wh + b)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    hidden_state = h0
    h = torch.empty((h0.shape[0], x.shape[1], h0.shape[1]), dtype=h0.dtype)
    
    for i in range(x.shape[1]):
        hidden_state = rnn_step_forward(x[:, i, :], hidden_state, Wx, Wh, b)
        h[:, i, :] = hidden_state
